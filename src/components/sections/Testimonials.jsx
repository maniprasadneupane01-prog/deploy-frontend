...
export default function Testimonials() {
  ...
  return (
    <section className="section bg-[var(--bg-section-alt)] overflow-hidden">
      ...
      <div className="flex gap-6 transition-transform duration-500 ease-in-out" style={{ transform: `translateX(-${offset * (380 + 24)}px)` }} onMouseEnter={() => paused.current = true} onMouseLeave={() => paused.current = false}>
        {doubled.map((item, i) => (
          <div key={i} className="flex-shrink-0 w-[380px] p-8 rounded-[var(--radius-lg)] bg-[var(--grad-card)] border border-[var(--border-subtle)]">
