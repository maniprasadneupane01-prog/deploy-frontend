...
export default function Services() {
  ...
  return (
    <section id="services-section" ref={sectionRef} className="section bg-[var(--bg-section-alt)]">
      ...
      <div ref={trackRef} className="flex gap-6 px-[var(--content-pad)] md:px-0 md:pl-[var(--content-pad)]">
        {iconKeys.map((key) => {
          const Icon = iconMap[key];
          const item = t(`services.items.${key}`, { returnObjects: true });
          return (
            <div key={key} data-service={item.name} className="flex-shrink-0 w-[340px] md:w-[380px] p-8 rounded-[var(--radius-xl)] bg-[var(--grad-card)] border border-[var(--border-subtle)] shadow-sm hover:translate-y-[-10px] hover:shadow-lg hover:border-terra-500/60 transition-all duration-300 group cursor-pointer" onClick={() => openPanel(item.name, document.activeElement)}>
              <div className="w-16 h-16 mb-6 text-terra-400 group-hover:rotate-[360deg] transition-transform duration-800"><Icon /></div>
