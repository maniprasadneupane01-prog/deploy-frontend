...
export default function Button({ variant = 'primary', children, className = '', magnetic = false, ...props }) {
  ...
  const variants = {
    primary: 'bg-terra-500 text-white px-8 py-4 shadow-brand hover:translate-y-[-2px] hover:shadow-brand-lg active:translate-y-0',
    ghost: 'bg-transparent border border-terra-500/50 text-terra-400 px-7 py-4 hover:border-terra-500 hover:bg-terra-500/[0.08]',
    text: 'bg-transparent text-terra-400 px-4 py-2 hover:text-terra-300',
    danger: 'bg-red-600 text-white px-8 py-4 hover:bg-red-700',
  };
